<script type="text/javascript">
	$(document).ready(function(){
		...
		if( $('#fish_buing').length > 0 ){
			var eChart_2 = echarts.init(document.getElementById('fish_buing'));
			var option1 = {
				tooltip : {
					backgroundColor: 'rgba(33,33,33,1)',
					borderRadius:0,
				...
				series : [
					{
						...
						data:[
							@foreach($fish_buing as $value)
								{value:{{ $value->total_price }}, name:'{{ $value->name }}'},
							@endforeach
						],
						itemStyle: {
							emphasis: {
	...
	$(function () {
	    ...
	    $('#dashboardCalendar').fullCalendar({
	      ...
	      events    : [
	      	@foreach($calenderData as $value)
		        {
		        	<?php
		        	$data = array('to'=>$value->created_at->toDateString(),'pond_id'=>"4"); 
		        	?>
	    ...
	    $('#dashboardHarvestCalendar').fullCalendar({
	      ...
	      //Random default events
	      events    : [
	      	@foreach($harvestCalenderData as $value)
		        {
		        	<?php
		        	$data = array('to'=>$value->created_at->toDateString(),'pond_id'=>"4"); 
		        	?>
